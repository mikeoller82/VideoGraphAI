...
class ImageGenerationAgent(Agent):
    ...
    async def execute(self, input_data: Dict[str, Any]) -> Any:
        ...
        for i, scene in enumerate(scenes):
            ...
            prompt = prompt = f"""
Generate bold, high-impact visuals based on the provided {visual_description} and {image_keyword}, ensuring each image is dynamic, precise, and instantly captivating for fast-paced YouTube Shorts. The visuals must align perfectly with the scene context, highlighting the environment, characters, actions, and mood as described. Prioritize details, eye-catching styles, striking compositions, and dynamic color schemes that stand out in a fast-scrolling feed. Ensure the images are accurate, and visually cohesive with the corresponding scene, with an emphasis on energy and attention-grabbing details ideal for viral content that will get massive amount of views on Youtubes platform."""
            try:
...
def generate_voiceover(scenes: List[Dict[str, Any]], output_file: str) -> bool:
    ...
    try:
        ...
        ref_audio = os.path.join(f5_tts_dir, "tests", "ref_audio", "adam.wav")
        ref_text = ""
    ...
    try:
        ...
        with open(input_text_file, "w", encoding="utf-8") as f:
            for scene in scenes:
                ...
                if text and text.lower() != 'none, no voiceover, no subtitles, just music, no specific text for this scene, no text':
                    f.write(text + " ")
